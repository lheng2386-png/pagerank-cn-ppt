...
Decision order:
1. Use Git Data API batch commits when --force-api is set and a token is available.
2. Use gh + git + SSH when the local machine is already authenticated.
3. Use Git Data API batch commits when a token is available.
4. Fall back to a dry-run manifest with clear next steps.
...
def upload_state_path(root: Path) -> Path | None:
    git_dir = run(["git", "rev-parse", "--git-dir"], root)
    if git_dir.returncode != 0:
        return None
    path = Path(git_dir.stdout.strip())
    if not path.is_absolute():
        path = root / path
    return path / "codex_upload_state.json"


def read_upload_state(root: Path, repo_full_name: str) -> dict:
    path = upload_state_path(root)
    ...
    try:
        ...
        return {}
    if state.get("repo") != repo_full_name:
        return {}
    return state


def write_upload_state(root: Path, repo_full_name: str, branch: str, commit: str) -> None:
    path = upload_state_path(root)
    if not path:
        return
    path.write_text(
...
def remote_branch_exists(root: Path, remote: str, branch: str) -> bool:
    return run(["git", "ls-remote", "--exit-code", "--heads", remote, branch], root, timeout=60).returncode == 0


def switch_to_upload_branch(root: Path, current_branch: str, target_branch: str, remote: str, check_remote: bool) -> bool:
    if target_branch == current_branch:
        return False
    if git_branch_exists(root, target_branch):
        require_ok(run(["git", "switch", target_branch], root, timeout=60), "switch to upload branch")
    elif check_remote and remote_branch_exists(root, remote, target_branch):
        require_ok(run(["git", "fetch", remote, f"{target_branch}:refs/remotes/{remote}/{target_branch}"], root, timeout=120), "fetch upload branch")
        require_ok(run(["git", "switch", "-c", target_branch, f"{remote}/{target_branch}"], root, timeout=60), "track upload branch")
    else:
        require_ok(run(["git", "switch", "-c", target_branch], root, timeout=60), "create upload branch")
    return True
...
def branch_upstream_remote(root: Path, branch: str) -> str:
    upstream = run(["git", "rev-parse", "--abbrev-ref", f"{branch}@{{upstream}}"], root)
    if upstream.returncode != 0 or "/" not in upstream.stdout.strip():
        return ""
    return upstream.stdout.strip().split("/", 1)[0]
...
def stage_manifest(root: Path, manifest: Manifest) -> list[str]:
    pruned_files = prune_excluded_tracked_files(root, manifest)
    rels = stageable_paths(root, manifest)
    if not rels and not pruned_files:
        return []
    ...
    return sorted(set(rels + pruned_files))


def prune_excluded_tracked_files(root: Path, manifest: Manifest) -> list[str]:
    if run(["git", "rev-parse", "--is-inside-work-tree"], root).returncode != 0:
        return []

    allowed = {path.relative_to(root).as_posix() for path in manifest.files}
    tracked = run(["git", "ls-files", "-z"], root, timeout=120)
    if tracked.returncode != 0:
        return []

    to_prune = sorted(path for path in tracked.stdout.split("\0") if path and path not in allowed)
    if not to_prune:
        return []

    with tempfile.NamedTemporaryFile("w", encoding="utf-8", delete=False) as handle:
        pathspec_file = Path(handle.name)
        handle.write("\0".join(to_prune))
        handle.write("\0")
    try:
        require_ok(
            run(
                ["git", "rm", "--cached", "--ignore-unmatch", "--pathspec-from-file", str(pathspec_file), "--pathspec-file-nul"],
                root,
                timeout=120,
            ),
            "prune excluded tracked files",
        )
    finally:
        pathspec_file.unlink(missing_ok=True)
    return to_prune
...
def stageable_paths(root: Path, manifest: Manifest) -> list[str]:
    all_paths = [path.relative_to(root).as_posix() for path in manifest.files]
    if run(["git", "rev-parse", "--is-inside-work-tree"], root).returncode != 0:
        return all_paths

    allowed = set(all_paths)
    status = run(["git", "status", "--porcelain=v1", "-z", "--untracked-files=all"], root, timeout=120)
    if status.returncode != 0:
        return all_paths

    changed: list[str] = []
    parts = [part for part in status.stdout.split("\0") if part]
    i = 0
    while i < len(parts):
        entry = parts[i]
        code = entry[:2]
...
def git_upload(
    ...
    upstream_remote = branch_upstream_remote(root, target_branch)
    should_avoid_current_branch = (
        target_branch == current_branch
        and upstream_remote == remote
        and branch_is_behind(root, target_branch)
    )
    if should_avoid_current_branch:
        target_branch = f"codex/upload-{time.strftime('%Y%m%d-%H%M%S')}"
        generated_branch = True
    switched_branch = switch_to_upload_branch(root, current_branch, target_branch, remote, check_remote=not generated_branch)
    staged_files = stage_manifest(root, manifest)
    ...
    result = {
        ...
        "started_on_branch": current_branch,
        "switched_branch": switched_branch,
        "remote": remote,
        "last_commit": head.stdout.strip(),
        "created_commit": committed,
        "files_in_upload": [path.relative_to(root).as_posix() for path in manifest.files],
        "files_staged": staged_files,
    }
    write_upload_state(root, repo_full_name, target_branch, head.stdout.strip())
    return result
...
def main() -> int:
    ...
    parser.add_argument("--continue-last-upload", action="store_true", help="Reuse the last Codex upload branch for this repo")
    ...
    parser.add_argument("--force-api", action="store_true", help="Use Git Data API uploads even when git+SSH is available")
    ...
    selected_branch = args.new_branch or args.branch
    upload_state = read_upload_state(root, args.repo) if args.continue_last_upload else {}
    if args.continue_last_upload:
        if not upload_state.get("branch"):
            raise SystemExit("No previous Codex upload branch found for this repo.")
        selected_branch = upload_state["branch"]
    ...
    chosen = (
        "git-data-api"
        if args.force_api and token
        else "dry-run-only"
        if args.force_api
        else "gh+git+ssh"
        if git_ready and source_git_ok
        else "git-data-api"
        if token
        else "dry-run-only"
    )
    ...
    summary = {
        ...
        "continued_from_state": bool(upload_state),
    ...
    if chosen == "gh+git+ssh":
        target_branch = selected_branch or git_current_branch(root)
        result = git_upload(root, owner, repo, manifest, args.message, target_branch, args.remote_name, args.private, args.repo)
    ...
    target_branch = selected_branch or base_branch
